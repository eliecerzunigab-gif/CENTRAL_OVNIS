...
def build_bars(d, mx, w='100px', cls=''):
    ...
    for n,c in sorted(d.items(), key=lambda x:-x[1]):
        ...
        r.append(f'<div class="bar-row"><span class="bar-label" style="width:{w}">{esc(n)}</span><div class="bar-track"><div class="bar-fill {cls}" style="width:{p:.0f}%"></div></div><span class="bar-count">{c}</span></div>')
    return '\n'.join(r)
...
def build_top10(recs):
    ...
    for i,r in enumerate(sorted(recs, key=lambda x:-x.get('score',0))[:10]):
        ...
        if img: ih=f'<img class="card-img" src="{esc(img)}" alt="{esc(r["title"])}" onclick="openModal(\'{esc(img)}\',\'{esc(r["title"])}\')">'
        else: ih='<div class="card-img-placeholder">🛸</div>'
        pl = r.get('pdf_link','')
        pb = f'<a href="{esc(pl)}" target="_blank" class="btn-pdf">📄 PDF</a>' if pl else ''
        dv=r.get('incident_date',''); lv=r.get('incident_location',''); dv2=r.get('description','')[:150]; sc=r.get('score',0)
        card=f'<div class="top-card"><div class="top-rank">{rank}</div><div class="score-badge">{sc}</div>{ih}<div class="card-body"><div class="card-type type-{r["type"]}">{r["type"]}</div><div class="card-title">{esc(r["title"])}</div><div class="card-meta"><span class="card-agency">{esc(r["agency"])}</span><span>{esc(dv)}</span><span>{esc(lv)}</span></div><div class="card-desc">{esc(dv2)}</div><div class="card-links">{pb}</div></div></div>'
        c.append(card)
    return '\n'.join(c)
...
def build_table(recs):
    ...
    for i,rec in enumerate(recs):
        ...
        r.append(f'<tr><td>{n}</td><td>{esc(t)}</td><td>{esc(a)}</td><td>{esc(d)}</td><td>{esc(l)}</td><td><span class="table-type {tp}">{tp}</span></td><td><span class="score-cell">{sc}</span></td></tr>')
    return '\n'.join(r)
...
.header h1{{font-family:'Orbitron',sans-serif;font-size:2.2em;font-weight:800;background:linear-gradient(135deg,var(--accent),var(--accent2));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:2px}}
.header .subtitle{{color:var(--text2);font-size:1em;margin-top:8px;letter-spacing:4px;text-transform:uppercase}}
.header .badge-row{{display:flex;justify-content:center;gap:15px;margin-top:15px;flex-wrap:wrap}}
.header .badge{{display:inline-flex;align-items:center;gap:6px;padding:6px 14px;border-radius:20px;font-size:12px;font-weight:600}}
...
.comp-card{{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:20px;text-align:center}}
.comp-card.r1{{border-color:rgba(59,130,246,0.3)}}
.comp-card.r2{{border-color:rgba(245,158,11,0.3)}}
.comp-card .release-label{{font-family:'Orbitron',sans-serif;font-size:14px;font-weight:700;letter-spacing:2px;margin-bottom:8px}}
.comp-card .big-number{{font-family:'Orbitron',sans-serif;font-size:3em;font-weight:800}}
.comp-card .sub-text{{font-size:12px;color:var(--text-muted);margin-top:4px}}
.comp-card .stats-row{{display:flex;justify-content:center;gap:20px;margin-top:12px;flex-wrap:wrap}}
.comp-card .stat-item{{text-align:center}}
.comp-card .stat-item .stat-number{{font-size:1.3em;font-weight:700;font-family:'Orbitron',sans-serif}}
.comp-card .stat-item .stat-label{{font-size:10px;color:var(--text-muted);text-transform:uppercase;letter-spacing:1px}}

/* VS DIVIDER */
.vs-divider{{display:flex;align-items:center;justify-content:center;gap:20px;margin:10px 0 30px}}
.vs-divider .line{{flex:1;height:1px;background:var(--border)}}
.vs-divider .vs-text{{font-family:'Orbitron',sans-serif;font-size:1.5em;font-weight:800;color:var(--accent);text-shadow:0 0 20px rgba(0,212,255,0.3);padding:0 10px}}

/* SECTION TITLE */
.section-title{{font-family:'Orbitron',sans-serif;font-size:1.3em;font-weight:700;color:var(--text);margin-bottom:20px;display:flex;align-items:center;gap:10px}}
...
/* SPLIT LAYOUT */
.split-grid{{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:40px}}
.split-card{{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:20px;overflow:hidden}}
.split-card.r1{{border-color:rgba(59,130,246,0.2)}}
.split-card.r2{{border-color:rgba(245,158,11,0.2)}}
.split-card .split-header{{display:flex;align-items:center;gap:10px;margin-bottom:15px;padding-bottom:10px;border-bottom:1px solid var(--border)}}
.split-card .split-header .split-icon{{width:10px;height:10px;border-radius:50%}}
.split-card .split-header .split-icon.r1{{background:var(--r1)}}
.split-card .split-header .split-icon.r2{{background:var(--r2)}}
.split-card .split-header .split-label{{font-family:'Orbitron',sans-serif;font-size:13px;font-weight:700;letter-spacing:1px}}
.split-card .split-header .split-count{{font-size:12px;color:var(--text-muted);margin-left:auto}}

/* AGENCY BARS */
.bars-container{{display:flex;flex-direction:column;gap:5px}}
.bar-row{{display:flex;align-items:center;gap:8px}}
.bar-label{{font-size:10px;color:var(--text2);width:100px;text-align:right;flex-shrink:0;line-height:1.2}}
.bar-track{{flex:1;height:16px;background:var(--bg2);border-radius:4px;overflow:hidden}}
...
.bar-fill.loc-fill{{background:linear-gradient(90deg,var(--accent4),rgba(16,185,129,0.4))}}
.bar-count{{font-size:10px;color:var(--text-muted);width:25px;text-align:left;flex-shrink:0}}

/* TOP 10 CARDS */
.top10-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:14px}}
...
.top-card:hover{{border-color:rgba(0,212,255,0.3);transform:translateY(-3px);box-shadow:0 12px 40px rgba(0,0,0,0.4)}}
.top-card .top-rank{{position:absolute;top:8px;left:8px;width:28px;height:28px;background:rgba(0,0,0,0.7);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;color:var(--accent);z-index:2;font-family:'Orbitron',sans-serif;border:1px solid rgba(0,212,255,0.3)}}
.top-card .score-badge{{position:absolute;top:8px;right:8px;padding:2px 8px;border-radius:10px;font-size:9px;font-weight:700;z-index:2;letter-spacing:1px;background:rgba(16,185,129,0.2);color:var(--accent4);border:1px solid rgba(16,185,129,0.3)}}
.card-img{{width:100%;height:160px;object-fit:cover;cursor:pointer;transition:transform .3s}}
.card-img:hover{{transform:scale(1.05)}}
.card-img-placeholder{{width:100%;height:160px;display:flex;align-items:center;justify-content:center;font-size:2.5em;background:linear-gradient(135deg,var(--bg2),var(--card))}}
.card-body{{padding:12px}}
.card-type{{display:inline-block;padding:2px 6px;border-radius:3px;font-size:9px;font-weight:700;letter-spacing:1px;margin-bottom:4px}}
...
.card-title{{font-size:13px;font-weight:600;color:var(--text);margin-bottom:4px;line-height:1.3}}
.card-meta{{display:flex;flex-wrap:wrap;gap:4px;font-size:10px;color:var(--text-muted);margin-bottom:6px}}
.card-meta .card-agency{{color:var(--accent);font-weight:600}}
.card-desc{{font-size:11px;color:var(--text2);line-height:1.4;margin-bottom:8px}}
.card-links{{display:flex;gap:6px}}
.btn-pdf{{display:inline-flex;align-items:center;gap:3px;padding:4px 10px;background:rgba(239,68,68,0.15);color:#ef4444;border-radius:5px;font-size:10px;font-weight:600;text-decoration:none;transition:all .2s}}
.btn-pdf:hover{{background:rgba(239,68,68,0.3)}}

/* TABLE */
.table-wrapper{{overflow-x:auto;max-height:500px;overflow-y:auto}}
.data-table{{width:100%;border-collapse:collapse;font-size:11px}}
.data-table th{{background:var(--bg2);color:var(--text-muted);padding:8px 10px;text-align:left;font-weight:600;text-transform:uppercase;letter-spacing:1px;font-size:9px;border-bottom:2px solid var(--border);position:sticky;top:0;z-index:2}}
.data-table td{{padding:6px 10px;border-bottom:1px solid var(--border);color:var(--text2)}}
.data-table tr:hover td{{background:var(--card-hover)}}
.table-type{{display:inline-block;padding:1px 5px;border-radius:3px;font-size:9px;font-weight:600}}
...
.score-cell{{font-family:'Orbitron',sans-serif;font-size:10px;color:var(--accent4);font-weight:700}}

/* SEARCH */
.search-box{{margin-bottom:12px}}
.search-box input{{width:100%;padding:10px 14px;background:var(--bg2);border:1px solid var(--border);border-radius:10px;color:var(--text);font-size:12px;font-family:'Inter',sans-serif;outline:none;transition:all .3s}}
...
.modal-overlay img{{max-width:90vw;max-height:85vh;border-radius:8px;object-fit:contain}}
.modal-close-btn{{position:absolute;top:20px;right:30px;font-size:30px;color:white;cursor:pointer;background:none;border:none;z-index:1001}}
.modal-title-text{{position:absolute;bottom:20px;left:50%;transform:translateX(-50%);color:white;font-size:14px;text-align:center;background:rgba(0,0,0,0.7);padding:8px 16px;border-radius:8px;max-width:80%}}

/* FOOTER */
.footer{{text-align:center;padding:30px;border-top:1px solid var(--border);color:var(--text-muted);font-size:12px;margin-top:20px}}
.footer a{{color:var(--accent);text-decoration:none}}

/* RESPONSIVE */
@media(max-width:900px){{.comparison-grid,.split-grid{{grid-template-columns:1fr}}.header h1{{font-size:1.5em}}.comp-card .big-number{{font-size:2em}}.bar-label{{width:70px;font-size:9px}}}}
</style>
</head>
<body>
<div class="container">
...
<div class="section-title"><span class="dot r1"></span> Agencias <span class="dot r2" style="margin-left:15px"></span></div>
...
<div class="section-title"><span class="dot r1"></span> TOP 10 · Release 1 <span class="dot r2" style="margin-left:15px"></span> TOP 10 · Release 2 (Nuevos)</div>
<div class="split-grid">
<div class="split-card r1">
...
<div class="section-title"><span class="dot r1"></span> Todos los Registros · Release 1 ({s1['total']}) <span class="dot r2" style="margin-left:15px"></span> Todos los Registros · Release 2 ({s2['total']})</div>
<div class="split-grid">
<div class="split-card r1">
